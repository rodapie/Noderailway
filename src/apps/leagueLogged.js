...
function redireccionarPartida(nombreInvocador, game) {
    window.location.href = "/leagueGame?nombreInvocador=" + encodeURIComponent(nombreInvocador) + "&gameId=" + encodeURIComponent(game);
}
let numeroPartida = 0;
...
function obtenerDatos(event){
    ...
    for(let i = 0; i < 10; i++) {
        if (event.target.classList.value.indexOf(i.toString()) !== -1) {
            ...
            redireccionarPartida(nombreInvocador, ids[i]);
        }
    }
}


async function quitarFavorito(nombre,tag,amigo){
    ...
    const response = await fetch("/cargarFavoritos",{
        ...
        body: JSON.stringify({ nombre,amigo })
    });

    const data = await response.json();
}

async function cargarFavoritos(nombre,amigo){
    console.log("cargando tags");
    const response = await fetch("/cargarFavoritos",{
        method: 'POST',
        headers: {
